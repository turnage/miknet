...
impl ConnectionManager {
    pub fn new() -> Result<Self> { Ok(Self { key: Key::new()?, connections: HashMap::new() }) }

    pub fn receive(&mut self, sender: SocketAddr, event: Event) -> Vec<(SocketAddr, Cmd)> {
        let (conn, cmds) = if let Some(conn) = self.connections.remove(&sender) {
            conn.step(event, sender)
        } else {
            Connection::Listen { key: self.key.clone() }.step(event, sender)
        };
        let cmds = conn.prepare_cmds(sender, cmds);
...
impl Connection {
    ...
    fn prepare_cmds(&self, peer: SocketAddr, cmds: Vec<Cmd>) -> Vec<(SocketAddr, Cmd)> {
        cmds.into_iter()
            .map(|cmd| {
                (
                    peer.clone(),
        ...
        match (self, event) {
            ...
            (Connection::Listen { key },
             Event::Chunk(Chunk::Init { token: their_token, tsn: their_tsn })) => {
                ...
                (
                    Connection::Listen { key: key.clone() },
                    vec![
    ...
    fn events_from(&self, gram: Gram) -> Vec<Event> {
        match *self {
            Connection::Listen { .. } => gram.events(0),
            Connection::InitSent { token, .. } => gram.events(token),
            Connection::CookieEchoed(tcb) => gram.events(tcb.their_token),
            Connection::Established(tcb) => gram.events(tcb.their_token),
            Connection::ShutdownSent { token, .. } => gram.events(token),
            Connection::ShutdownAckSent { token, .. } => gram.events(token),
            Connection::Failed => Vec::new(),
        }
    }
